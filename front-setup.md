# 專案安裝步驟：Vite + React + TypeScript + Tailwind v4 + shadcn/ui
...
將 `<your-app-name>` 替換為你的專案名稱（僅限英文、數字、連字號）：
...
## Step 7：修改 `tsconfig.app.json`

> ⚠️ 注意：是 `tsconfig.app.json`，不是 `tsconfig.json`
...
互動式問答依提示選擇即可，若詢問 component library 選 **Radix**。
...
| `No import alias found in your tsconfig.json` | 確認 `baseUrl` 與 `paths` 是加在 `tsconfig.app.json`，而非 `tsconfig.json` |
| `Cannot find module 'path'` | 執行 `npm install -D @types/node` |
| `shadcn init 失敗` | 確認使用 `npx shadcn@latest init -t vite`（必須加上 `-t vite` 旗標） |
